...
public class CenturyGUI {
    ...
    static {
        ...
        to2.put(Material.COBBLESTONE, 640);
        to2.put(Material.OAK_PLANKS, 320);
        COSTS.put(2, to2);

        Map<Material, Integer> to3 = new HashMap<>();
        to3.put(Material.IRON_INGOT, 320);
        to3.put(Material.COAL, 160);
        COSTS.put(3, to3);

        Map<Material, Integer> to4 = new HashMap<>();
        to4.put(Material.GOLD_INGOT, 320);
        to4.put(Material.REDSTONE, 160);
        COSTS.put(4, to4);

        Map<Material, Integer> to5 = new HashMap<>();
        to5.put(Material.DIAMOND, 160);
        to5.put(Material.OBSIDIAN, 80);
        COSTS.put(5, to5);

        Map<Material, Integer> to6 = new HashMap<>();
        to6.put(Material.EMERALD, 160);
        to6.put(Material.LAPIS_LAZULI, 160);
        COSTS.put(6, to6);

        Map<Material, Integer> to7 = new HashMap<>();
        to7.put(Material.NETHERITE_INGOT, 40);
        to7.put(Material.ANCIENT_DEBRIS, 80);
        COSTS.put(7, to7);

        Map<Material, Integer> to8 = new HashMap<>();
        to8.put(Material.BEACON, 10);
        to8.put(Material.NETHER_STAR, 10);
        COSTS.put(8, to8);
    }
    ...
    public static int getRequiredCenturyForMining(Material ore) {
        switch (ore) {
            ...
            case IRON_ORE: case DEEPSLATE_IRON_ORE: return 3;
            case COPPER_ORE: case DEEPSLATE_COPPER_ORE: return 2;
            case GOLD_ORE: case DEEPSLATE_GOLD_ORE: return 4;
            case REDSTONE_ORE: case DEEPSLATE_REDSTONE_ORE: return 4;
            case LAPIS_ORE: case DEEPSLATE_LAPIS_ORE: return 4;
            case DIAMOND_ORE: case DEEPSLATE_DIAMOND_ORE: return 5;
            case EMERALD_ORE: case DEEPSLATE_EMERALD_ORE: return 6;
            case ANCIENT_DEBRIS: return 7;
            default: return 1;
        }
    }
                ...
                if (level == currentCentury + 1) {
                    ...
                    if (cost != null) {
                        for (Map.Entry<Material, Integer> e : cost.entrySet()) {
                            lore.add("§7- " + getMaterialName(e.getKey()) + ": §f" + e.getValue());
                        }
                    }
        ...
        for (Map.Entry<Material, Integer> entry : cost.entrySet()) {
            if (!storage.contains(entry.getKey(), entry.getValue())) {
                player.sendMessage("§cНедостаточно ресурсов на складе города. Требуется: " +
                        getMaterialName(entry.getKey()) + " x" + entry.getValue());
                return false;
            }
        }
        for (Map.Entry<Material, Integer> entry : cost.entrySet()) {
